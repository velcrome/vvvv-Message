...
{
    /// <summary>
    /// Contains an extension methods for dynamic queries against IEnumerable objects instead of just IQueryable objects
    /// Written by Jon von Gillern
    /// See http://blog.nitriq.com for more information
    /// http://blog.nitriq.com/HowToCreateALinqStatementFromAString.aspx
    /// </summary>
    public static class DynamicLinqExtensions
    {
        ...
        {
            return items.OrderBy<T>(property, true);
        }
        ...
        {
            return items.OrderBy<T>(property, false);
        }

        private static IEnumerable<T> OrderBy<T>(this IEnumerable<T> items, string property, bool ascending)
        {
            var comparer = new ComparerWrapper<T>(CreateComparer<T>(property, ascending));
            return items.OrderBy(ix => ix, comparer);
        }
        ...
        {
            return CreateComparer<T>(propertyName, true);
        }
        ...
        public static Func<T, T, int> CreateComparer<T>(string propertyName, bool ascending)
        {
            ...
            if (ascending)
                text = string.Format("(item1.{0} == item2.{0}) ? 0 : (item1.{0} > item2.{0}) ? 1 : -1", propertyName);
            else
                text = string.Format("(item2.{0} == item1.{0}) ? 0 : (item2.{0} > item1.{0}) ? 1 : -1", propertyName);

            //this was the original way of doing this, but unfortunately Nullable<T> doesn't
            ...
            var expression = DynamicExpression.ParseLambda(parameters, typeof(int), text);
            Delegate del = expression.Compile();
        ...
        {
            var expression = DynamicExpression.ParseLambda(typeof(TInput), typeof(TOutput), expressionString, new object[1]);
            Delegate del = expression.Compile();
            Func<TInput, TOutput> pred = del as Func<TInput, TOutput>;
